...
LevelIdentify <- function(object, L0Min = NA, L0Max = NA, L1Min = NA, L1Max = NA) {
  ...
  if(anyNA(c(L1Min, L1Max))) {
    ...
    pADen$x2 <- object[pA > L0Min & pA < L0Max, median(Sm)] * c(1 - 0.1)
    pADen$x3 <- object[pA > L0Min & pA < L0Max, median(Sm)] * c(1 - 0.25)
    tx <- highlight_key(pADen)
    widgets <- bscols(
      widths = c(12),
      filter_select("adjust", "Adjust", tx, ~ adjust, multiple = F)
    ...
    if(pd == "YES") {
      L1Min <- object[pA > L0Min & pA < L0Max, median(Sm)] * c(1 - 0.25)
      L1Max <- object[pA > L0Min & pA < L0Max, median(Sm)] * c(1 - 0.1)
    } else {
